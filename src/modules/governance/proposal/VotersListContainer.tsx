...
export const VotersListContainer = (props: VotersListProps): JSX.Element => {
  ...
  const getVoterInfo = async () => {
    ...
    try {
      ...
      // Fetch voters from The Graph subgraph instead of API
      // GraphQL query to fetch votes for a proposal
      const query = `
        query ProposalVotes($proposalId: String!) {
          voteEmitteds(where: {AaveGovernanceV2_id: $proposalId}) {
            id
            voter
            support
            votingPower
      ...
      // The proposalId from props is a number, but subgraph stores it as stringified BigInt
      // So we need to make sure it's a string
      const variables = {
        proposalId: proposalId.toString(),
      };

      const resp = await fetch(governanceConfig.queryGovernanceDataUrl, {
        method: 'POST',
      ...
      if (resp.ok) {
        ...
        // data.voteEmitteds is an array of votes
        // Map to GovernanceVoter type
        const yaes: GovernanceVoter[] = [];
        const nays: GovernanceVoter[] = [];
        for (const vote of data.voteEmitteds) {
          const voter: GovernanceVoter = {
            address: vote.voter,
            ensName: null, // ENS not available from subgraph
            votingPower: Number(vote.votingPower),
            twitterAvatar: null, // Not available from subgraph
