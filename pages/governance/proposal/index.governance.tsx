...
export default function DynamicProposal() {
  ...
  console.log('[Nahman] DynamicProposal(): id', id);
  ...
  async function initialize(_ipfsGateway: string) {
    ...
    console.log('[Nahman] DynamicProposal(): values', values);
    console.log('[Nahman] DynamicProposal(): rest', rest);
    
    const proposal = await enhanceProposalWithTimes(rest);
    console.log('[Nahman] DynamicProposal(): proposal', proposal);
  ...
  useEffect(() => {
    initialize(governanceConfig.ipfsGateway);
  }, [id]);
