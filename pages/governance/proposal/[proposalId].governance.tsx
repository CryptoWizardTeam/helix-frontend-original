...
export async function getStaticProps({ params }: { params: { proposalId: string } }) {
  console.log('[Nahman] getStaticProps called with params:', params);
  ...
  try {
    ...
    console.log('[Nahman] getStaticProps: Fetching proposal ID:', proposalId);
    
    const proposal = ProposalFetcher.get(proposalId);
    console.log('[Nahman] getStaticProps: Proposal fetched:', !!proposal);
    
    const ipfs = IpfsFetcher.get(proposalId);
    console.log('[Nahman] getStaticProps: IPFS fetched:', !!ipfs);
    ...
    console.error(`[Nahman] getStaticProps Error fetching proposal ${params.proposalId}:`, error);
    return {
      props: {
        proposal: null,
        ipfs: null,
...
export default function ProposalPage({
  ...
  console.log('[Nahman] updateProposal(): initialProposal', initialProposal);
  console.log('[Nahman] updateProposal(): ipfs', ipfs);
  console.log('[Nahman] updateProposal(): proposalId', proposal?.id);

  async function updateProposal() {
    if (!proposal) return;
    const { values, ...rest } = await governanceContract.getProposal({ proposalId: proposal.id });
    console.log('[Nahman] updateProposal(): values', values);
    console.log('[Nahman] updateProposal(): rest', rest);
